...
async function generateOne(opts = {}) {
  ...
  let idx = { skipped: true };
  if (full.status === 'published') { try { idx = await require('../indexnow').submit([`/blog/${full.slug}`, '/blog', '/']); } catch (e) { idx = { ok: false, error: e.message }; } }
  return { post: db.prepare('SELECT * FROM posts WHERE id=?').get(post.id), autoPublished: autoPublish, inblog: ib, indexnow: idx, type };
}
