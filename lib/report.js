...
function collect(week) {
  ...
  const shots = shot.pairs();
  return { week, range, posts, clubs, priceUp, inq, traffic, prevTraffic, audit: lastAudit, auditHistory: audit.history(), tasks, doneCount, next, generatedAt: kstDate(), kickoff: kickoff(), shots };
}

function shotsHtml(d) {
  const sh = d.shots; if (!sh || (!sh.pairs.length && !sh.admin.length)) return '';
  const cell = (f, cap) => f ? `<figure style="margin:0"><img src="${shot.dataUri(f)}" alt="${esc(cap)}" style="width:100%;border:1px solid #d9dee7;border-radius:6px"><figcaption class="muted">${esc(cap)}</figcaption></figure>` : `<div class="muted" style="border:1px dashed #d9dee7;border-radius:6px;padding:40px;text-align:center">이미지 없음</div>`;
  const rows = sh.pairs.map(p => `<tr><th colspan="2" style="text-align:left">${esc(p.label)}</th></tr><tr><td style="width:50%;vertical-align:top">${cell(p.before, '이전 — 기존 hanamarket.co.kr')}</td><td style="width:50%;vertical-align:top">${cell(p.after, `이후 — 신규 홈페이지 (${sh.afterDir ? path.basename(sh.afterDir) : ''} 캡처)`)}</td></tr>`).join('');
  ...
  return `<h2>0. 전후 비교 (구 홈페이지 → 신규 홈페이지)</h2><p class="muted">같은 페이지를 같은 해상도(데스크톱 960×600 기준)로 캡처. 좌: 기존 hanamarket.co.kr, 우: 신규 홈페이지 최신 캡처.</p><table style="table-layout:fixed">${rows}</table>${admin}`;
}
...
async function buildDocx(d, kind, html) {
  ...
  const shotRows = (d.shots && d.shots.pairs.length) ? d.shots.pairs.map(p => new TableRow({ children: [new TableCell({ children: [P(`${p.label} — 이전(hanamarket.co.kr)`, { bold: true, size: 18 }), p.before ? img(p.before) : P('이미지 없음')] }), new TableCell({ children: [P(`${p.label} — 이후(신규 홈페이지)`, { bold: true, size: 18 }), p.after ? img(p.after) : P('이미지 없음')] })] })) : [];
  ...
  const children = [
    ...
    ...(shotRows.length ? [H('0. 전후 비교 (구 홈페이지 → 신규 홈페이지)'), P('같은 페이지를 같은 해상도로 캡처. 좌: 기존 사이트, 우: 신규 홈페이지 최신 캡처.', { color: '667085', size: 18 }), new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: shotRows })] : []),
