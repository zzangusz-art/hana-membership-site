...
// 현재 요청의 origin(https://호스트). 렌더가 동기라 미들웨어에서 세팅한 값을 그대로 쓴다. og:image 등 "공유용 절대 URL"에만 사용(canonical은 SITE_URL 고정).
let REQ_ORIGIN = '';
function setRequestOrigin(o) { REQ_ORIGIN = o || ''; }
function shareOrigin() { return REQ_ORIGIN || settings.siteUrl(); }
...
const NAV = [
  ...
  { label: '하나회원권TV메뉴얼', href: '/videos', children: [
    { label: '공식 유튜브 영상', href: '/videos' }] },
...
function page({ title, description, path, jsonld = [], body, breadcrumbs = [], ogImage, noindex = false, bodyClass = '', extraHead = '', dateModified }) {
  ...
  const og = ogImage ? (ogImage.startsWith('http') ? ogImage : shareOrigin() + ogImage) : shareOrigin() + '/img/og.png?v=3';
  const ogTitle = title.length > 60 ? title.slice(0, 58) + '…' : title;
  const ogDesc = description.length > 80 ? description.slice(0, 78).replace(/[,·\s]+$/, '') + '…' : description; // 네이버 사이트 간단 체크 기준 80자
  return `<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8">
...
<meta property="og:title" content="${attr(ogTitle)}"><meta property="og:description" content="${attr(ogDesc)}"><meta property="og:url" content="${attr(url)}"><meta property="og:image" content="${attr(og)}"><meta property="og:image:secure_url" content="${attr(og)}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="${attr(ogTitle)}">
<meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${attr(ogTitle)}"><meta name="twitter:description" content="${attr(ogDesc)}"><meta name="twitter:image" content="${attr(og)}">
...
<header class="header" id="header">
  <div class="topbar"><div class="wrap"><span class="tb-phone">📞 <a href="tel:${attr(s.phone)}"><b>${esc(s.phone)}</b></a> · 24시간 상담</span><span class="tb-badges" aria-label="소속·계열"><span class="tbb"><img src="/img/kgmma.png" alt="한국골프회원권경영인협회(KGMMA) 정회원사" width="104" height="22"></span><span class="tbb"><img src="/img/hana-membership-tour.png" alt="하나멤버쉽투어" width="76" height="24"></span></span><span class="tb-right"><a href="${attr(s.youtube)}" target="_blank" rel="noopener">유튜브</a><a href="/apply">매매 신청</a><a href="/about/location">오시는 길</a></span></div></div>
  <div class="wrap header-inner">
    ...
    ${navHtml(path)}
    <div class="header-cta"><a class="btn btn-call impact" href="tel:${attr(s.phone)}" aria-label="전화 상담 ${attr(s.phone)}"><span class="ring" aria-hidden="true"></span><span class="ic" aria-hidden="true">📞</span><span class="num">${esc(s.phone)}</span></a><a class="btn btn-primary" href="/apply">매매 신청</a></div>
  ...
  <div class="mobile-nav" id="mobileNav" role="dialog" aria-label="메뉴" aria-modal="true"><div class="dh"><b>메뉴</b><button class="dclose" id="drawerClose" aria-label="메뉴 닫기">×</button></div>${NAV.map(n => `<details><summary>${n.label}</summary><ul>${n.children.map(c => `<li><a href="${c.href}">${c.label}</a></li>`).join('')}</ul></details>`).join('')}<a class="btn btn-primary block" href="/apply">매매 신청</a></div>
</header>
...
<section class="cta-band"><div class="wrap cta-inner"><div><h2>회원권 매수·매도, 시세부터 명의개서까지 한 번에</h2><p>2004년부터 해 온 일입니다. 시세 확인, 계약, 명의개서, 등록 뒤 부킹 문의까지 담당자 한 사람이 맡습니다.</p></div><div class="cta-actions"><a class="btn btn-light big-phone impact" href="tel:${attr(s.phone)}"><span class="ic" aria-hidden="true">📞</span> ${esc(s.phone)}</a><a class="btn btn-green" href="/apply">매매 신청하기</a></div></div></section>
<footer class="footer">
...
<div class="float-cta"><a href="${attr(s.kakao_channel || '/apply')}" class="fc-kakao"${s.kakao_channel ? ' target="_blank" rel="noopener"' : ' data-nokakao="1"'} aria-label="카카오톡 문의"><svg viewBox="0 0 24 24" width="22" height="22" aria-hidden="true"><path fill="#191919" d="M12 3C6.48 3 2 6.58 2 11c0 2.84 1.87 5.33 4.68 6.74L5.6 21.5c-.1.36.3.65.6.44l4.5-3.02c.42.05.86.08 1.3.08 5.52 0 10-3.58 10-8S17.52 3 12 3z"/></svg><span>카카오톡 문의</span></a><a href="tel:${attr(s.phone)}" class="fc-call" aria-label="전화 상담">📞</a><a href="/apply" class="fc-apply">매매 신청</a><button class="fc-top" id="toTop" aria-label="맨 위로">↑</button></div>
<script src="/js/site.js?v=${ASSET_STAMP}" defer></script>
</body>
</html>`;
}

module.exports = { page, orgLd, websiteLd, breadcrumbLd, faqLd, NAV, setStamp, setRequestOrigin, shareOrigin };
