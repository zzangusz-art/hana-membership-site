...
<section class="section"><div class="wrap"><h2>${e.h1} 매물 ${items.length}건</h2>${items.length ? `<div class="lst-grid">${items.map(l => `<div class="lst reveal${l.image ? ' has-img' : ''}" id="l${l.id}">${l.image ? `<a class="lst-img" href="/listings/${l.id}"><img src="${attr(l.image)}" alt="${attr(l.title)}" loading="lazy" width="600" height="600"></a>` : ''}<span class="tag">${esc(l.kind || '')}</span><h3><a href="/listings/${l.id}">${esc(l.title)}</a></h3><p>${esc(l.region || '')}${l.price ? ' · ' + fmtMan(l.price) : ''}</p>${l.body ? `<p class="lst-body">${esc(truncate(l.body, 120))}</p>` : ''}<div class="lst-btns"><a class="btn btn-ghost" href="/listings/${l.id}">상세</a><a class="btn btn-primary" href="/apply?item=${encodeURIComponent(l.name || l.title)}">문의</a></div></div>`).join('')}</div>` : `<p class="note">현재 등록된 매물이 없습니다. 원하시는 조건을 <a href="/apply">매매 신청</a>으로 남겨 주시면 매물이 나오는 즉시 연락드립니다.</p>`}
...
router.get('/listings/:id(\\d+)', (req, res, next) => {
  const l = db.prepare("SELECT * FROM listings WHERE id=? AND status<>'deleted'").get(req.params.id); if (!l) return next();
  const s = settings.all(); const site = settings.siteUrl();
  let images = []; let info = []; try { images = JSON.parse(l.images || '[]'); } catch (_) { /* no-op */ } try { info = JSON.parse(l.info_json || '[]'); } catch (_) { /* no-op */ }
  if (!images.length && l.image) images = [l.image];
  // 값이 비었거나 제목과 같거나, 아래 설명 본문에 그대로 나오는 항목은 요약표에서 제외
  info = info.filter(([k, v]) => v && String(v).trim() && String(v).trim() !== l.title && !(l.desc_html && /골프장 소개|피트니스 소개|회원권 종류|회원 혜택|^기타$/.test(k)));
  const related = db.prepare("SELECT id,title,image,kind,price FROM listings WHERE status='open' AND category=? AND id<>? ORDER BY id DESC LIMIT 4").all(l.category, l.id);
  const catLabel = LCAT[l.category] || '매물'; const excl = Object.entries(EXCL).find(([, e]) => e.kind === l.kind);
  const crumb = excl ? [{ name: '매물·전용관', href: '/listings' }, { name: excl[1].h1, href: '/exclusive/' + excl[0] }] : [{ name: '매물·전용관', href: '/listings' }, { name: catLabel, href: '/listings?category=' + l.category }];
  const bluf = `${l.title}, ${l.kind && l.kind !== '매물' ? l.kind + ' ' : ''}${catLabel} 상품입니다. ${l.price ? '표시 가격 ' + fmtMan(l.price) + '(만원 기준, 시점에 따라 변동). ' : ''}${info.filter(([k]) => /회원권 종류|회원 혜택/.test(k)).map(([k, v]) => k + ': ' + truncate(v, 80)).join(' · ')}${info.length ? ' ' : ''}상세 조건·부대비용은 ${s.phone}으로 문의하시면 당일 안내합니다.`;
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">${esc(excl ? excl[1].h1 : catLabel)}</p><h1>${esc(l.title)}</h1><p class="bluf">${esc(bluf)}</p></div></section>
<section class="section"><div class="wrap prd"><div class="prd-main">${images.map((im, i) => `<figure class="prd-fig${i ? ' reveal' : ''}"><img src="${attr(im)}" alt="${attr(l.title)}${images.length > 1 ? ' ' + (i + 1) : ''}" ${i ? 'loading="lazy"' : ''} width="1080" height="1080"></figure>`).join('')}${l.desc_html ? `<article class="prose prd-desc">${l.desc_html}</article>` : ''}</div>
<aside class="prd-side"><div class="side-card"><span class="tag">${esc(l.kind || catLabel)}</span><h2>${esc(l.title)}</h2>${info.length ? `<table class="prd-info">${info.map(([k, v]) => `<tr><th>${esc(k)}</th><td>${esc(v)}</td></tr>`).join('')}</table>` : ''}${l.price ? `<p class="prd-price"><b>${fmtMan(l.price)}</b><small>표시 시점 기준 · 변동 가능</small></p>` : ''}<a class="btn btn-primary block" href="/apply?item=${encodeURIComponent(l.name || l.title)}">이 상품 상담 신청</a><a class="btn btn-call block impact" href="tel:${attr(s.phone)}"><span class="ic" aria-hidden="true">📞</span> ${esc(s.phone)}</a><p class="note">24시간 전화 상담 · 평일 09:00–18:00 방문 상담</p></div>
${related.length ? `<div class="side-card"><h3>같은 분류 상품</h3><ul class="rel-list">${related.map(r => `<li><a href="/listings/${r.id}">${r.image ? `<img src="${attr(r.image)}" alt="" loading="lazy" width="56" height="56">` : ''}<span>${esc(r.title)}${r.price ? `<small>${fmtMan(r.price)}</small>` : ''}</span></a></li>`).join('')}</ul></div>` : ''}</aside></div></section>`;
  const ld = [{ '@context': 'https://schema.org', '@type': 'Product', name: l.title, image: images.map(im => site + im), description: bluf, category: catLabel, brand: { '@type': 'Brand', name: l.name || l.title }, offers: { '@type': 'Offer', url: `${site}/listings/${l.id}`, priceCurrency: 'KRW', ...(l.price ? { price: l.price * 10000 } : {}), availability: 'https://schema.org/InStock', seller: { '@id': site + '/#org' } } }];
  res.send(page({ title: `${l.title}: ${excl ? excl[1].h1 : catLabel} 상품 안내`, description: truncate(bluf, 155), path: `/listings/${l.id}`, body, breadcrumbs: [...crumb, { name: l.title, href: `/listings/${l.id}` }], ogImage: images[0] || undefined, jsonld: ld, dateModified: isoFromTs(l.updated_at) }));
});
...
router.get('/about', (req, res) => {
  ...
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">회사소개</p><h1>${esc(s.legal_name)} 소개</h1><p class="bluf">${esc(s.legal_name)}는 2004년부터 골프·콘도·피트니스 회원권을 중개해 온 회사입니다. 회원권 상담과 매매가 본업이고, 분양 대행 28건, 골프장·리조트 예약 알선, 해외 골프투어도 합니다. 하나카드 '하나멤버스'나 하나금융그룹과는 관계가 없습니다.</p></div></section>
<section class="section"><div class="wrap club-grid2"><div class="club-main"><article class="prose">
<h2>회사 소개</h2><p>서울 강남구 압구정에 있습니다. 2004년 에이원회원권거래소로 시작해 2008년 비전회원권거래소와 합치고 2014년 지금 이름이 됐습니다. 대표이사 ${esc(s.ceo)}(연세대학교 경제대학원, 전 비전회원권거래소·에이원회원권거래소 대표이사)가 ${years}년째 회원권을 중개하고 있습니다.</p>
<h2>하는 일</h2><table><thead><tr><th>서비스</th><th>내용</th></tr></thead><tbody><tr><td>회원권 상담</td><td>예산에 맞는 종목과 그 골프장의 실제 부킹 사정 안내</td></tr><tr><td>회원권 매매</td><td>계약서·정산 내역서 발급, 골프장 명의개서 접수와 등록 확인</td></tr><tr><td>회원권 분양</td><td>신규 분양 회원권의 분양가·입회금 반환 조건 안내, 분양 대행(2004년부터 28건)</td></tr><tr><td>예약 알선</td><td>골프장·리조트 예약 대행</td></tr><tr><td>해외 골프투어</td><td>일정·티타임·숙소 수배(계열사 하나멤버쉽투어)</td></tr></tbody></table>
<h2>다른 거래소와 다른 점</h2><ul><li><strong>정확한 시세</strong>: 골프 ${g.total}종목 시세를 매주 실거래·호가로 갱신하고 홈페이지에 공개합니다.</li><li><strong>분양 실적</strong>: 블루버드·서원밸리·엘리시안 강촌·리베라·레이크힐스·프리스틴밸리·크리스탈밸리·썬밸리·서울드래곤시티 등 골프장·리조트·피트니스 분양 대행.</li><li><strong>파트너십</strong>: 한국골프회원권경영인협회, 네오위즈, 미래신용정보, 동양생명, 삼양, 포스코ICT, 한글과컴퓨터, 서울드래곤시티 등과 협력.</li><li><strong>담당자 고정</strong>: 처음 상담한 사람이 계약, 명의개서, 등록 뒤 부킹 문의까지 맡습니다.</li></ul>
...
<section class="section tl2-sec"><div class="wrap narrow"><h2 class="sr-h">연도별 전체 실적</h2>
...
router.get('/about/careers', (req, res) => {
  const s = settings.all();
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">회사소개</p><h1>채용 안내</h1><p class="bluf">회원권 상담과 매매 중개를 할 분을 수시로 뽑습니다. 골프를 좋아하고 전화 응대가 편한 분이면 됩니다.</p></div></section><section class="section"><div class="wrap narrow prose"><h2>어떤 일을 하나요?</h2><ul><li>회원권 매수·매도 고객 상담 및 시세 안내</li><li>계약·정산·명의개서 진행 지원</li><li>골프장·리조트 파트너 커뮤니케이션</li></ul><h2>지원 방법</h2><p>이력서를 <a href="mailto:${attr(s.email)}">${esc(s.email)}</a>로 보내 주시거나 ${esc(s.phone)}으로 문의해 주세요. 서류 검토 후 면접 일정을 안내드립니다.</p></div></section>`;
...
router.get('/videos', (req, res) => {
  ...
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">커뮤니티</p><h1>하나회원권거래소 유튜브</h1><p class="bluf">공식 채널 <a href="${attr(s.youtube)}" target="_blank" rel="noopener">하나회원권TV(@hanamarket)</a>는 구독자 1만 4천여 명, 영상 448편(2026년 9월 기준)의 회원권 전문 채널입니다. 월별 골프회원권 시세 전망, 골프장 소개, 회원권 리스크 해설 영상을 올립니다. 홈페이지 시세표와 같이 보시면 됩니다.</p></div></section><section class="section"><div class="wrap">${videos.length ? `<div class="video-grid">${videos.map(v => `<div class="video reveal"><div class="yt" data-id="${attr(v.youtube_id)}"><img src="https://i.ytimg.com/vi/${attr(v.youtube_id)}/hqdefault.jpg" alt="${attr(v.title)}" loading="lazy" width="480" height="360"><button class="yt-play" aria-label="${attr(v.title)} 재생">▶</button></div><h2>${esc(v.title)}</h2>${v.description ? `<p>${esc(truncate(v.description, 120))}</p>` : ''}</div>`).join('')}</div>` : `<p class="note">영상 목록을 준비 중입니다. <a href="${attr(s.youtube)}" target="_blank" rel="noopener">유튜브 채널</a>에서 바로 보실 수 있습니다.</p>`}</div></section>`;
