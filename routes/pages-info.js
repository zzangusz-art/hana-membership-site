...
router.get('/videos', (req, res) => {
  ...
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">커뮤니티</p><h1>하나회원권거래소 유튜브</h1><p class="bluf">공식 채널 <a href="${attr(s.youtube)}" target="_blank" rel="noopener">하나회원권TV(@hanamarket)</a>는 구독자 1만 4천여 명, 영상 448편(2026년 9월 기준)의 회원권 전문 채널입니다. 월별 골프회원권 시세 전망, 골프장 소개, 회원권 리스크 해설 영상을 홈페이지 시세표와 함께 보시면 판단에 도움이 됩니다. 홈페이지의 시세표·골프장 소개와 함께 보시면 판단에 도움이 됩니다.</p></div></section><section class="section"><div class="wrap">${videos.length ? `<div class="video-grid">${videos.map(v => `<div class="video reveal"><div class="yt" data-id="${attr(v.youtube_id)}"><img src="https://i.ytimg.com/vi/${attr(v.youtube_id)}/hqdefault.jpg" alt="${attr(v.title)}" loading="lazy" width="480" height="360"><button class="yt-play" aria-label="${attr(v.title)} 재생">▶</button></div><h2>${esc(v.title)}</h2>${v.description ? `<p>${esc(truncate(v.description, 120))}</p>` : ''}</div>`).join('')}</div>` : `<p class="note">영상 목록을 준비 중입니다. <a href="${attr(s.youtube)}" target="_blank" rel="noopener">유튜브 채널</a>에서 바로 보실 수 있습니다.</p>`}</div></section>`;
