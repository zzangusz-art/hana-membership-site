...
<p class="note">RSS: <a href="/rss.xml">/rss.xml</a>${settings.cfg('inblog_url') ? ` · 블로그: <a href="${attr(settings.cfg('inblog_url'))}" target="_blank" rel="noopener">${esc(settings.cfg('inblog_url'))}</a>` : ''}</p></div></section>`;
  res.send(page({ title: `${type ? TL[type] : '골프회원권 시세 변동 리포트·거래 가이드'} ${total}건, 매일 발행`, description: `골프회원권 시세 변동 주간 리포트, 구매·판매·명의개서·세금 가이드, 골프장별 소개, 인기 종목 동향 ${total}건.`, path: '/blog', body, breadcrumbs: [{ name: '시세 리포트·가이드', href: '/blog' }], ogImage: '/og/page/blog.png', jsonld: [{ '@context': 'https://schema.org', '@type': 'Blog', name: '하나회원권거래소 시세 리포트·가이드', url: settings.siteUrl() + '/blog', publisher: { '@id': settings.siteUrl() + '/#org' }, blogPost: rows.slice(0, 10).map(p => ({ '@type': 'BlogPosting', headline: p.title, url: `${settings.siteUrl()}/blog/${p.slug}`, datePublished: isoFromTs(p.published_at) })) }] }));
});
...
router.get('/blog/:slug', (req, res, next) => {
  ...
  const body = `<section class="page-head post-head"><div class="wrap narrow"><p class="eyebrow"><a href="/blog?type=${attr(p.type)}">${TL[p.type] || '가이드'}</a></p><h1>${esc(p.title)}</h1><p class="post-meta"><span>작성 ${esc(p.author || '하나회원권거래소')}</span><time datetime="${isoFromTs(p.published_at)}">발행 ${fmtKoDate(pubDate)}</time>${modDate !== pubDate ? `<time datetime="${isoFromTs(p.updated_at)}">수정 ${fmtKoDate(modDate)}</time>` : ''}</p>${p.excerpt ? `<p class="bluf">${esc(p.excerpt)}</p>` : ''}</div></section>
...
<div class="author-box"><img src="/img/mark.png" alt="" width="48" height="48"><div><b>${esc(s.legal_name)} 편집팀</b><p>하나회원권거래소가 자체 시세표와 상담 사례를 바탕으로 씁니다. 세금과 법률 부분은 세무사·변호사 확인을 권합니다.</p></div></div>
